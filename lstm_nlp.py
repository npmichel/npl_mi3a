...
from tensorflow.keras.layers import Embedding, LSTM , Dense # type: ignore # Couches du reseau
...
# 1 : Tokenization
def lstm_tokenization(sentences):
    """  
    Process LSTM process
    """
...
# 2 Data preparation
def lstm_data_preparation(sentenses_indeces):
    """ Model process """
    ...
    y = X[:, -1] # Mot final a predire
    X = X[:, :-1] # Sequence d'entree
    return X, y, max_len

# 3 Model preparation
def lstm_model(vocab_size, max_len):
    """LSTM model
    Définition du réseau neurenal a memoire long terme
    """
    ...
    model = Sequential([
        ...
        LSTM(16), # Couche memoire longue controlee, 
        # La valeur 16 correspond à la dimension de l’état mémoire, et non au nombre de motspris en compte.
...
# 4. Model training
def fit_lstm_model(model, x_data, y_data):
    """
...
if __name__ == "__main__":
    ...
    print(f" LTM")
    # 1. Tokenizer
    tokenizer, vocab_size = lstm_tokenization(sentences=sentenses)
    setences_indices = tokenizer.texts_to_sequences(sentenses)

    #2. Data 
    x_data, y_data, max_len = lstm_data_preparation (sentenses_indeces=setences_indices)

    # 3. Model 
    model = lstm_model(vocab_size=vocab_size, max_len=max_len)

    # 4. Training
    model = fit_lstm_model(model=model, x_data=x_data, y_data=y_data)

    # Prediction
    test_sentence = ["le", "chat", "que", "le", "chien", "que", "le", "voisin", "que", "l", "enfant", "regarde"]
    ...
    print(f"Mot predit par LSTM =>: {predicted_word}")
